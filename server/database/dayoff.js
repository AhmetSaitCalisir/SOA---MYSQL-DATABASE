...
router.get("/list/id/:id", (req, res) => {
    let sql = `select i.ID,cb.Ad,cb.Soyad,b.BirimAd,u.UnvanAdi,i.BaslangicTarih,i.BitisTarih,y.Ad as 'onaylayan ad',y.Soyad as 'onaylayan soyad' from ((((calisanbilgi cb join izinler i on i.KisiID=cb.CalisanID)
  join birimler b on b.BirimID=i.BirimID) 
  join unvan u on u.UnvanID=cb.UnvanID)
  join yonetici y on y.CalisanID=i.OnaylayanID) where cb.CalisanID=${req.params.id}`;
...
router.delete('/delete/id/:id', (req, res) => {
    let sql = `delete from izinler where KisiID=${req.params.id}`;
    db.query(sql, (err, result) => {
        if (err) throw err;
        console.log(`${req.params.id} id'li çalışanın izni silindi. `);
    })
})
